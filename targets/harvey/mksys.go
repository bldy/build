...
func (mkSys *MkSys) Build() error {
	logger := log.New(&mkSys.buf, "", log.Lmicroseconds)
	logger.Println(prettyprint.AsJSON(mkSys))
	outfile := os.Stdout
	if mkSys.Mode != "syscallfiles" && mkSys.OutPath != "" {
		of, err := os.Create(mkSys.OutPath)
		if err != nil {
			err := fmt.Errorf("os create %s:%s", mkSys.OutPath, err.Error())
	...
	if err != nil {
		err := fmt.Errorf("os readAll %s:%s", mkSys.SysConf, err.Error())
		logger.Print(err.Error())
		return err
	}
	...
	case "sys_harvey.s":
		if mkSys.ARCH != "amd64" {
			logger.Println("ARCH unsupported or not set")
		}
...
`)
	...
	case "syscallfiles":
		if mkSys.ARCH != "amd64" {
			logger.Fatal("ARCH unsupported or not set")
		}
...
`)
		...
		for i := range syscalls {
			...
			path := path.Join(mkSys.OutPath, syscalls[i].Libname+".s")
